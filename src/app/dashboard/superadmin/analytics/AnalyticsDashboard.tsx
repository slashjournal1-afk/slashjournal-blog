...
function ReportUnavailable({ name, code, detail }: { name: string; code?: string; detail?: string }) {
  return <div className="border border-[var(--warning)]/30 bg-[var(--warning-soft)] p-4 text-sm"><div className="flex items-center gap-2"><CircleAlert className="h-4 w-4 text-[var(--warning)]" /> {name} belum tersedia.</div><p className="mt-2 text-xs text-[var(--text-muted)]">Kode: {code || 'UNKNOWN'}{detail ? ` - ${detail}` : ''}</p></div>;
}
