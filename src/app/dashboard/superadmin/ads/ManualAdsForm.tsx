...
const slotLabels = {
  leaderboard: 'Banner Horizontal',
  in_feed: 'In-Feed Horizontal',
  sidebar_sticky: 'Sidebar Vertikal',
} as const;
...
export function ManualAdsForm({ initialSlots }: { initialSlots: AdSlot[] }) {
  ...
  async function save(slot: AdSlot, form: HTMLFormElement) {
    ...
    try {
      ...
      setMessage(`Slot ${slotLabels[slot.slotName as keyof typeof slotLabels]} berhasil disimpan.`);
    } catch (error: unknown) {
  ...
  return (
    <div className="space-y-8">
      <header className="border-b border-[var(--border-color)] pb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-[var(--text-muted)]">Kelola creative brand untuk tiga placement internal. Saat nonaktif atau belum tersedia, placement memakai dummy image dan mengarah ke kontak iklan.</p>
      </header>
        ...
        {slots.map((slot) => (
          <form key={slot.slotName} onSubmit={(event) => { event.preventDefault(); void save(slot, event.currentTarget); }} className="space-y-4 border border-[var(--border-color)] bg-[var(--bg-card)] p-5">
            <div className="flex items-start justify-between gap-3">
              <div><h2 className="font-bold">{slotLabels[slot.slotName as keyof typeof slotLabels] || slot.slotName}</h2><p className="mt-1 text-xs text-[var(--text-muted)]">{slot.slotName}</p></div>
              <label className="flex items-center gap-2 text-xs"><input name="isActive" type="checkbox" defaultChecked={slot.isActive} /> Aktif</label>
            </div>
            <div className="relative aspect-[16/7] overflow-hidden border border-[var(--border-color)] bg-[var(--bg-card-muted)]">
            ...
            <label className="block text-xs font-semibold">Gambar creative<input name="image" type="file" accept="image/*" className="mt-1 block w-full text-xs" /></label>
